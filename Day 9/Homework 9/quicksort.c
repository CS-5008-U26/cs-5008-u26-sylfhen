...
/* forward declaration for the quicksort helper function */
void quicksort_helper(int a[], int start, int end);
/* sortarray - sorts an array of ints using quicksort */
...
/* quicksort_helper - the actual quicksort implementation */
/* sorts a[start..end] in place using the last element as the pivot */
void quicksort_helper(int a[], int start, int end) {
    if (start >= end) {
        return;
    }
    ...
    int pivot = a[end];
    int separator = start - 1;    // index of the last element known to be smaller than the pivot
    ...
    // do recursive calls on left and right halves
